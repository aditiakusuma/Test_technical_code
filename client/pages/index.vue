<script setup lang="ts">
    ...
    const generate = async (type: 'odd' | 'prima' | 'triangle') => {
        try {
            const res = await $fetch(`/generate?type=${type}`, { baseURL: 'http://localhost:3333'})
            result.value = res
        } catch (error) {
...
<div style=" padding: 80px;">
    <input v-model="input" type="text" style=" margin-bottom: 20px" placeholder="Input Angka">
    <div>
